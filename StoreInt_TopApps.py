...
prefix_columns = ['start_date', 'granularity', 'market', 'device', 'country']

# Element name for main contents
response_main_element = 'list'

# Need to set the exact same name as in API response
columns = ['rank', 'category', 'feed', 'product_id',
           'product_code', 'product_name', 'product_category',
           'release_date', 'price', 'publisher_id',
           'publisher_name', 'company_id', 'company_name',
           'unified_product_id', 'unified_product_name',
           'product_franchise_id', 'product_franchise_name',
           'parent_company_id', 'parent_company_name', 'estimate',
           'change_percent', 'change_rank', 'change_value'
           ]
...
parser = argparse.ArgumentParser(
    description='Extracting App Annie Store Intelligence Top Apps data \
                 via App Annie API')
parser.add_argument(
...
filename = "result_StoreInt_TopApps_" + query_market + \
