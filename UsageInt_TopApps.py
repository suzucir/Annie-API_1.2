...
prefix_columns = ['start_date', 'granularity', 'market', 'device',
...
columns = ['usage_rank', 'product_id', 'product_code', 'product_name',
           'product_category', 'publisher_id',
           'publisher_name', 'company_id', 'company_name',
           'unified_product_id', 'unified_product_name',
           'parent_company_id', 'parent_company_name', 'usage_penetration',
           'active_users', 'install_penetration', 'open_rate',
           'total_minutes', 'total_sessions', 'avg_sessions_per_user',
           'avg_session_duration', 'avg_time_per_user', 'avg_active_days',
           'mb_per_user', 'mb_per_session',
           'change_usage_penetration', 'change_percent_active_users',
           'change_install_penetration', 'change_open_rate'
           ]
...
parser.add_argument(
    ...
    metavar="GRANULARITY", default="monthly",
    help="specify data granularity (Options	monthly | weekly)")
parser.add_argument(
...
filename = "result_Usage_TopApps_" + opts.opts_device + \
...
while (end_date - run_date).days + 1 > 0:
    ...
    if opts.opts_granularity == "daily":
        run_date = run_date + datetime.timedelta(days=1)
    elif opts.opts_granularity == "weekly":
        run_date = run_date + datetime.timedelta(days=7)
    elif opts.opts_granularity == "monthly":
        run_date = annieapi.next_month(run_date)
